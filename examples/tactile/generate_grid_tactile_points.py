...
def select_grid_points(points, normals, grid_spacing=0.01, grid_spacing_h=None, grid_spacing_v=None,
    ...
    5. For each slice, place points with uniform horizontal spacing

    Args:
        points: (N, 3) dense sampled points
        normals: (N, 3) surface normals
        grid_spacing: target distance between adjacent grid points (in meters), used as fallback
        grid_spacing_h: horizontal spacing between grid points (in meters), overrides grid_spacing for columns
    ...
    if grid_spacing_h is None:
        grid_spacing_h = grid_spacing
    if grid_spacing_v is None:
    ...
    # Horizontal axis = Z × avg_normal (perpendicular to both)
    h_axis = np.cross(z_axis, avg_normal)
    h_axis_norm = np.linalg.norm(h_axis)

    if h_axis_norm < 1e-6:
        # avg_normal is parallel to Z, fall back to X-axis as horizontal
        h_axis = np.array([1.0, 0.0, 0.0])
    else:
        h_axis = h_axis / h_axis_norm

    # Project points to 2D: (horizontal, vertical) = (dot with h_axis, Z coordinate)
    points_h = points @ h_axis  # horizontal coordinate
    points_v = points[:, 2]     # vertical coordinate (Z)
    points_2d = np.stack([points_h, points_v], axis=1)

    # height_dim = 1 (vertical/Z), width_dim = 0 (horizontal)
    height_dim = 1
    width_dim = 0
    ...
    if num_rows is not None:
        ...
        n_rows = max(3, int(np.round(height / grid_spacing_v)))
    ...
    # Use grid_spacing_h to determine number of global columns
    global_width = grid_max[width_dim] - grid_min[width_dim]

    if num_cols is not None:
        n_global_cols = num_cols
    else:
        n_global_cols = max(1, int(np.round(global_width / grid_spacing_h)))
...
def main():
    ...
    parser.add_argument("--grid-spacing", type=float, default=None,
                        help="Target spacing between grid points in meters (e.g., 0.01 for 1cm). "
                             ...
                             "Can be overridden by --grid-spacing-h and --grid-spacing-v.")
    parser.add_argument("--grid-spacing-h", type=float, default=None,
                        help="Horizontal spacing between grid points in meters. "
                             "Overrides --grid-spacing for columns (width direction).")
    parser.add_argument("--grid-spacing-v", type=float, default=None,
                        help="Vertical spacing between grid points in meters. "
                             "Overrides --grid-spacing for rows (height direction).")
    ...
    # Determine effective spacing values
    use_spacing = args.grid_spacing is not None or args.grid_spacing_h is not None or args.grid_spacing_v is not None
    if use_spacing:
        # Default to grid_spacing if specific h/v not provided
        effective_spacing_h = args.grid_spacing_h if args.grid_spacing_h is not None else args.grid_spacing
        effective_spacing_v = args.grid_spacing_v if args.grid_spacing_v is not None else args.grid_spacing

        if effective_spacing_h == effective_spacing_v:
            print(f"Grid spacing: {effective_spacing_h*1000:.1f}mm (adaptive to geometry)")
        else:
    ...
    tactile_data = {
        'hand_type': args.hand,
        'sampling_method': 'palm_grid_adaptive' if use_spacing else 'palm_grid',
        'grid_spacing': args.grid_spacing,
        'grid_spacing_h': effective_spacing_h if use_spacing else None,
        'grid_spacing_v': effective_spacing_v if use_spacing else None,
        'grid_rows': args.grid_rows if not use_spacing else None,
        'grid_cols': args.grid_cols if not use_spacing else None,
        'z_min': args.z_min,
        'z_max': args.z_max,
        'links': {}
    }
        ...
        for geom_idx, geom in enumerate(link.geoms):
            ...
            if use_spacing:
                ...
                grid_points, grid_normals, grid_indices = select_grid_points(
                    ...
                    grid_spacing_v=effective_spacing_v
                )
            else:
                # Fixed grid size
                grid_points, grid_normals, grid_indices = select_grid_points(
